  ...
  .then((stats) => {
    if (stats.data && stats.data.link_karma) {
      ...
      const metrics: SocialMetric[] = [
        ...
        { label: 'Joined', value: new Date(created_utc * 1000).getFullYear() },
      ];
      return metrics;
    }
    return [];
  ...
  .then((stats) => {
    if (stats.items && stats.items[0]) {
      ...
      const metrics: SocialMetric[] = [
        ...
        { label: 'Joined', value: new Date(creation_date * 1000).getFullYear() },
      ];
      return metrics;
    }
    return [];
...
export const _getKeybaseInfo = async (username: string) => {
  ...
  .then((stats) => {
    if (stats && stats.them && stats.them[0]) {
      ...
      const metrics: SocialMetric[] = [
        { label: 'Joined', value: new Date(basics.ctime * 1000).getFullYear() },
...
export const load = async () => {
  const { // Get usernames from config
    Twitter: twitterUn,
    Reddit: redditUn,
    GitHub: githubUn,
  ...
  const [twitter, reddit, github, stackoverflow, devto, codersrank, mastodon, keybase] = await Promise.all([
